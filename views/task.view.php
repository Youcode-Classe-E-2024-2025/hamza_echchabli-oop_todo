...
<div class="cnt min-w-[400px]  w-[96%] lg:w-[1024px]  mx-auto mt-20 mb-28  overflow-x-auto p-3">
            <div class="content  items-start  gap-3 ">
               ...
               <?php
if ($_SESSION['loginU'][1] == 'a') {
    echo '<button type="submit" class="bg-gray-400 hover:bg-gray-600 transition-colors w-full py-1 mt-1 mb-2 text-white">Update</button>';
}
?>
 </form>
               ...
               <?php foreach($task['assignees'] as $assignee) :?>
                <div class="w-full mt-2 bg-slate-200 px-4 flex justify-between font-semibold items-center" >
                    ...
                    <?php
if ($_SESSION['loginU'][1] == 'a') {
    echo '<button type="submit" class="bg-red-500 rounded-full flex justify-center items-center w-5 h-5">-</button>';
}
?> 
</form>
                </div>
                ...
                <?php
if ($_SESSION['loginU'][1] == 'a') {
               echo ' <button class="w-full btn-add-assignee mt-2 bg-slate-200 hover:bg-slate-400 transition-colors px-4 text-center text-2xl font-extrabold ">+</button>';
            }
            ?>  
